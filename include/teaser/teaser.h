...
public:
   ...
   */
  RegistrationSolution solve(const pcr::PointCloud& src_cloud,
  ...
  void reset(const Params& params) {
    ...
    if (params_.estimate_scaling) {
      setScaleEstimator(
          std::make_unique<pcr::TLSScaleSolver>(params_.noise_bound, params_.cbar2));
    } else {
      setScaleEstimator(
          std::make_unique<pcr::ScaleInliersSelector>(params_.noise_bound, params_.cbar2));
    }

    // Initialize the rotation estimator
    pcr::GNCRotationSolver::Params rotation_params{
    ...
    case ROTATION_ESTIMATION_ALGORITHM::GNC_TLS: { // GNC-TLS method
      setRotationEstimator(std::make_unique<pcr::GNCTLSRotationSolver>(rotation_params));
      break;
    }
    case ROTATION_ESTIMATION_ALGORITHM::FGR: { // FGR method
      setRotationEstimator(std::make_unique<pcr::FastGlobalRegistrationSolver>(rotation_params));
      break;
    }
    case ROTATION_ESTIMATION_ALGORITHM::QUATRO: { // Quatro method
      setRotationEstimator(std::make_unique<pcr::QuatroSolver>(rotation_params));
      break;
    }
    }
    ...
    setTranslationEstimator(
        std::make_unique<pcr::TLSTranslationSolver>(params_.noise_bound, params_.cbar2));
