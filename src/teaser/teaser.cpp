...
void pcr::ScalarTLSEstimator::estimate(const Eigen::RowVectorXd& X,
...
void pcr::ScalarTLSEstimator::estimate_tiled(const Eigen::RowVectorXd& X,
...
void pcr::FastGlobalRegistrationSolver::solveForRotation(
  ...
  double src_diameter = pcr::utils::calculateDiameter<double, 3>(src);
  double dest_diameter = pcr::utils::calculateDiameter<double, 3>(dst);
  ...
  *rotation = Eigen::Matrix3d::Identity(3, 3); // rotation matrix
  ...
  for (size_t i = 0; i < params_.max_iterations; ++i) {
    ...
    *rotation = pcr::utils::svdRot(src, dst, l_pq);
...
void pcr::QuatroSolver::solveForRotation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst, Eigen::Matrix3d* rotation,
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {
  assert(rotation);                 // make sure R is not a nullptr
  ...
  *rotation = Eigen::Matrix3d::Identity();
  ...
  for (size_t i = 0; i < params_.max_iterations; ++i) {
    ...
    rotation_2d = pcr::utils::svdRot2d(src_2d, dst_2d, weights);
...
void pcr::TLSScaleSolver::solveForScale(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
...
void pcr::ScaleInliersSelector::solveForScale(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
...
void pcr::TLSTranslationSolver::solveForTranslation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
...
pcr::RobustRegistrationSolver::RobustRegistrationSolver(
    const pcr::RobustRegistrationSolver::Params& params) {
  reset(params);
}

Eigen::Matrix<double, 3, Eigen::Dynamic>
pcr::RobustRegistrationSolver::computeTIMs(const Eigen::Matrix<double, 3, Eigen::Dynamic>& v,
...
pcr::RegistrationSolution
pcr::RobustRegistrationSolver::solve(const pcr::PointCloud& src_cloud,
                                        const pcr::PointCloud& dst_cloud,
...
pcr::RegistrationSolution
pcr::RobustRegistrationSolver::solve(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                        const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst) {
  ...
  if (params_.inlier_selection_mode != INLIER_SELECTION_MODE::NONE) {
    ...
    pcr::MaxCliqueSolver::Params clique_params;

    if (params_.inlier_selection_mode == INLIER_SELECTION_MODE::PMC_EXACT) {
      clique_params.solver_mode = pcr::MaxCliqueSolver::CLIQUE_SOLVER_MODE::PMC_EXACT;
    } else if (params_.inlier_selection_mode == INLIER_SELECTION_MODE::PMC_HEU) {
      clique_params.solver_mode = pcr::MaxCliqueSolver::CLIQUE_SOLVER_MODE::PMC_HEU;
    } else {
      clique_params.solver_mode = pcr::MaxCliqueSolver::CLIQUE_SOLVER_MODE::KCORE_HEU;
    }
    ...
    pcr::MaxCliqueSolver clique_solver(clique_params);
...
double pcr::RobustRegistrationSolver::solveForScale(
...
Eigen::Vector3d pcr::RobustRegistrationSolver::solveForTranslation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& v1,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& v2) {
...
Eigen::Matrix3d pcr::RobustRegistrationSolver::solveForRotation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& v1,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& v2) {
...
void pcr::GNCTLSRotationSolver::solveForRotation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst, Eigen::Matrix3d* rotation,
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {
  assert(rotation);                 // make sure R is not a nullptr
  ...
  for (size_t i = 0; i < params_.max_iterations; ++i) {
    ...
    *rotation = pcr::utils::svdRot(src, dst, weights);
