#include "fpfh/feature_matcher.h"

namespace pcr {

std::vector<std::pair<int, int>> FeatureMatcher::CalculateCorrespondences(PointCloudType &source_cloud,
    ...
    for (auto &f : source_features) {
        Eigen::Matrix<float, Eigen::Dynamic, 1> fpfh(source_descriptor_size);
    ...
    for (auto &f : target_features) {
        Eigen::Matrix<float, Eigen::Dynamic, 1> fpfh(target_descriptor_size);
...
template <typename T>
void FeatureMatcher::BuildKDTree(const std::vector<T> &data, KDTree *tree) {
    int rows, dim;
...
template <typename T>
void FeatureMatcher::SearchKDTree(FeatureMatcher::KDTree *tree, const T &input, std::vector<int> &indices, 
...
void FeatureMatcher::NormalizePoints(bool use_absolute_scale) {
...
void FeatureMatcher::AdvancedMatching(bool use_crosscheck, bool use_tuple_test, float tuple_scale) {
    ...
    if (use_tuple_test && tuple_scale != 0) {
        ...
        for (int i = 0; i < number_of_trial; i++) {
            ...
            Eigen::Vector3f pti0 = {pointcloud_[fi][idi0].x, pointcloud_[fi][idi0].y, pointcloud_[fi][idi0].z};
            Eigen::Vector3f pti1 = {pointcloud_[fi][idi1].x, pointcloud_[fi][idi1].y, pointcloud_[fi][idi1].z};
            Eigen::Vector3f pti2 = {pointcloud_[fi][idi2].x, pointcloud_[fi][idi2].y, pointcloud_[fi][idi2].z};
            ...
            Eigen::Vector3f ptj0 = {pointcloud_[fj][idj0].x, pointcloud_[fj][idj0].y, pointcloud_[fj][idj0].z};
            Eigen::Vector3f ptj1 = {pointcloud_[fj][idj1].x, pointcloud_[fj][idj1].y, pointcloud_[fj][idj1].z};
            Eigen::Vector3f ptj2 = {pointcloud_[fj][idj2].x, pointcloud_[fj][idj2].y, pointcloud_[fj][idj2].z};
